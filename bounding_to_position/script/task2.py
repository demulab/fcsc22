    ...
    def bbox_callback(self, darknet_bboxs):
        ...
        if len(bboxs) != 0:
            ...
            for i, bb in enumerate(bboxs):
                """if bboxs[i].Class == 'Cooling sheet':
                    self.bbox = bboxs[i]
                elif bboxs[i].Class == 'Refresher':
                    self.bbox = bboxs[i]"""

                """if bboxs[i].Class == 'Plaster':
                    self.bbox = bboxs[i]"""

                if bboxs[i].Class == 'Scrubbing brush':
                    self.bbox = bboxs[i]

                elif bboxs[i].Class == 'Chocolate Snack':
                    self.bbox = bboxs[i]

                elif bboxs[i].Class == 'Tomato Soup':
                    self.bbox = bboxs[i]

                """if bboxs[i].Class == 'Detergent':
                    self.bbox = bboxs[i]"""

                """if bboxs[i].Class == 'Bond':
                    self.bbox = bboxs[i]"""

                """if bboxs[i].Class == 'Green tea':
                    self.bbox = bboxs[i]"""

                """if bboxs[i].Class == 'UNKNOWN1':
                    self.bbox = bboxs[i]"""

                """if bboxs[i].Class == 'UNKNOWN2':
                    self.bbox = bboxs[i]
                elif bboxs[i].Class == 'Cooling sheet':
                    self.bbox = bboxs[i]"""

                """if bboxs[i].Class == 'UNKNOWN3':
                    self.bbox = bboxs[i]"""
