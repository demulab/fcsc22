...
class XArm_command(object):
    ...
    def setting_pose(self, trans, heights):
        ...
        self.pose = geometry_msgs.msg.Pose()
         #self.q = tf.transformations.quaternion_from_euler(0, (math.pi / 3)*0.3 , math.pi)
        if heights == "high":
            self.q = tf.transformations.quaternion_from_euler(0, (math.pi/ 3)*0.7, math.pi)
        else:
    ...
    def bring_to_box(self):
        ...
        self.box_trans =  [[0.30, 0.0, 0.53 + add_height], [0.15, 0.20, 0.40 + add_height], [0.0, 0.35, 0.45 + add_height], [-0.24, 0.0, 0.45]]
    ...
    def look_shelf(self, step, side):
        if step == "low":
            if side =="left":
                ...
                self.setting_pose([0.23, 0.17, 0.48], "low")
            if side == "right":
                #self.setting_pose([0.30, 0, 0.48])
                self.setting_pose([0.23, -0.17, 0.48], "low")
        elif step == "middle":
            if side =="left":
                ...
                self.setting_pose([0.22, 0.17, 0.60], "middle")
            if side == "right":
                #self.setting_pose([0.30, 0, 0.59])
                self.setting_pose([0.22, -0.17, 0.60], "middle")
        elif step == "high":
            if side == "left":
                ...
                self.setting_pose([0.17, 0.15, 0.65], "high")
            if side == "right":
                #self.setting_pose([0.30, 0, 0.70])
                self.setting_pose([0.17, -0.15, 0.65],"high")
    ...
    def get_optimul_ar(self, ids):
        ...
        for data in ids:
            ...
            try:
                ...
                print(ar)
                self.listener.waitForTransform("/camera_link", ar, rospy.Time(), rospy.Duration(3.0))
    ...
    def vacuum_picking(self, ar_id):
        ...
        self.target_pose.position.z = trans[2] + 0.4#(10 * up_point[2][0])
        self.target_pose.orientation.x = self.q[0]#rot[0]
        self.target_pose.orientation.y = self.q[1]#rot[1]
        self.target_pose.orientation.z = self.q[2]#rot[2]
        self.target_pose.orientation.w = self.q[3]#rot[3]
        ...
        self.q = rot
        # approach 2
        self.target_pose.position.x = trans[0] + (7 * up_point[0][0]) #- 0.01#(1.89 * up_point[0][0]) - 0.02
        self.target_pose.position.y = trans[1] + (7 * up_point[1][0]) #- 0.03#(1.89 * up_point[1][0]) - 0.04
        self.target_pose.position.z = trans[2] + (7 * up_point[2][0])
        self.target_pose.orientation.x = self.q[0]#rot[0]
        self.target_pose.orientation.y = self.q[1]#rot[1]
        self.target_pose.orientation.z = self.q[2]#rot[2]
        self.target_pose.orientation.w = self.q[3]#rot[3]
        ...
        #final approach
        self.target_pose.position.x = trans[0] + (6 * up_point[0][0]) #- 0.01#(1.89 * up_point[0][0]) - 0.02
        self.target_pose.position.y = trans[1] + (6 * up_point[1][0]) #- 0.03#(1.89 * up_point[1][0]) - 0.04
        self.target_pose.position.z = trans[2] + (6 * up_point[2][0])
        self.target_pose.orientation.x = self.q[0]#rot[0]
        self.target_pose.orientation.y = self.q[1]#rot[1]
        self.target_pose.orientation.z = self.q[2]#rot[2]
        self.target_pose.orientation.w = self.q[3]#rot[3]
