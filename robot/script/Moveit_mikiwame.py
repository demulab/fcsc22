...
class Spawner(Manipulation):
    ...
    def mikiwame_base(self):
        ...
        s_pose.pose.position.z = -0.555 -0.32
    ...
    def call_other_boards(self):
        for i in range(2, 7):
            self.shelf_board_mesh(i, 1 , 0, -0.555 -0.32 - (0.175 * (i-2)), 0, 0, -1, 1)
    ...
    def open_shelf(self, step):
        if step == "low":
            ...
            self.shelf_board_mesh(1, 0.7 , 0, -0.555 -0.32 -(0.35*2), 0, 0, -1, 1)
            self.call_other_boards()
        elif step == "middle":
            self.remove_object("shelf_board_1")
            self.shelf_board_mesh(1, 0.7 , 0, -0.555 -0.32 - 0.35, 0, 0, -1, 1)
            self.call_other_boards()
        elif step == "high":
            self.remove_object("shelf_board_1")
            self.shelf_board_mesh(1, 0.7, 0, -0.555 -0.32, 0, 0, -1, 1)
            self.call_other_boards()
        else:
            pass
